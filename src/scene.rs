...
{
    (
        SceneObject::new(material, Box::new(shape.clone()), transform),
        aabb(shape, &transform),
    )
}

#[allow(dead_code)]
pub fn cornell_box_scene() -> (BVT<SceneObject, AABB<f32>>, Camera) {
...
pub fn structured_art_scene() -> (BVT<SceneObject, AABB<f32>>, Camera) {
    ...
    hitables.push(create_bvt_tuple(
        ...
        &Ball::new(1.0f32),
        Isometry::new(Vector::new(1.0, 1.0, -2.0), Vector::z()),
    ));
    hitables.push(create_bvt_tuple(
